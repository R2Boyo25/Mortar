        ...
        if (gincludes.find(filevec) != gincludes.end() && filename != "") {
            for (string &line : split(content, '\n')) {
                if (startsWith(line, "#include")) {
                    ...
                    if (fname[0] != '<') {
                        fname = strip(fname, "\"");
                        if (!(std::find(gincludes[filevec].begin(), gincludes[filevec].end(), getPath(dirName(filename), fname)) != gincludes[filevec].end())) {
                            gincludes[filevec].push_back(getPath(dirName(filename), fname));
                            for (string& f : includes(getPath(dirName(filename), fname), filevec)) {
